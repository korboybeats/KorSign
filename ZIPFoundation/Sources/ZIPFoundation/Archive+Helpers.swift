...
extension Archive {
    ...
    func readCompressed(entry: Entry, bufferSize: Int, skipCRC32: Bool, useZlib: Bool = false,
                        progress: Progress? = nil, with consumer: Consumer) throws -> CRC32 {
        let size = entry.centralDirectoryStructure.effectiveCompressedSize
        guard size <= .max else { throw ArchiveError.invalidEntrySize }
        if useZlib {
            return try Data.inflateImport(size: size, expectedSize: entry.uncompressedSize,
                bufferSize: bufferSize, skipCRC32: skipCRC32,
                provider: { _, count in try Data.readChunk(of: count, from: self.archiveFile) },
                consumer: { data in
                    if progress?.isCancelled == true { throw ArchiveError.cancelledOperation }
                    try consumer(data)
                    progress?.completedUnitCount += Int64(data.count)
