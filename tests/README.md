...
This guide describes each fixture's scope, prerequisites and limitations.
Individual standalone fixtures may need fewer SDK features than the app.
...
is deleted before Files reopens. Check full and partial batches separately.
Failed or unverified installs must retain the signed copy.
...
presentation behavior require separate physical-device testing.
...
explicit close, late events, retry and rejected-open fallback.
...
plain files. Real UIKit/HTTP lifetimes,
...
signing, updater network requests or application settings are used. Certificate-picker
UI and full backup-restore interactions require physical-device testing.
...
No sockets, requests or application settings are used. WebDAV clients and real
listener failures require integration testing.
