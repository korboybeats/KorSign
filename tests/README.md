...
is deleted before Files reopens. F15 is fixed, and full/partial batch cleanup was user-confirmed; do not treat
those checks as coverage of every failure or unverified-result path. Failed or unverified installs must
...
signing, updater network requests or application settings are used. The user confirmed selection retention across imports and restart, and reported
backup restore working. Deleted-selection recovery and exhaustive restore
interactions remain unverified on the phone.
...
No sockets, requests or application settings are used. The user confirmed browser login/password changes and disabled controls.
WebDAV client behavior and deterministic startup failures still need validation.

## Audit-complete candidate check run (2026-09-09)

The recorded audit-complete candidate passed 13 focused scripts: installation_archive,
...
the korsign default, migration of the literal ryuk username, and preservation
of other saved usernames.
