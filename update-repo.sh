...
jq --slurpfile releases "$release" '
  ...
  if ($release.draft or ($release.tag_name | test("^v[0-9]+(\\.[0-9]+){2}(-r[1-9][0-9]*)?$") | not))
  ...
  .apps |= map(
    ...
    .version = ($release.tag_name | ltrimstr("v") | split("-r")[0]) |
    .buildVersion = ($release.tag_name | split("-r") | .[1] // "0") |
    .versionDate = $release.published_at |
    .size = $asset.size |
    .downloadURL = $asset.browser_download_url |
    .versions = [{version: .version, buildVersion: .buildVersion, date: .versionDate, size: .size, downloadURL: .downloadURL}]
