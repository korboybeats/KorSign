...
struct BackupView: View {
	...
	@State private var _exportURL: TemporaryExport?
	...
	var body: some View {
		...
		.sheet(isPresented: $_showExporter, onDismiss: { _exportURL = nil }) {
			if let url = _exportURL {
				DocumentExporterView(urls: [url.url], retaining: url).ignoresSafeArea()
			}
		}
	...
	private func _presentRestoreComplete() {
		...
		let message = needsRestart
			? summary.lines.joined(separator: "\n") + "\n\n" + .localized("KorSign will restart to apply the backup.")
			: summary.lines.joined(separator: "\n")
