...
struct AppearanceView: View {
	...
	@AppStorage("Feather.selectedTintTheme")
	private var _selectedTintTheme: String = AppTintTheme.defaultTheme.rawValue

	@AppStorage("Feather.customTintColor")
	private var _customColorHex: String = Color.defaultUserTintHex

	@AppStorage("Feather.userTintColor")
	private var _effectiveColorHex: String = Color.defaultUserTintHex

	private var _tintColorBinding: Binding<Color> {
		Binding(
			get: { Color(hex: _customColorHex) },
			set: {
				let hex = $0.userTintHex()
				_customColorHex = hex
				_activateCustomTint(hex)
			}
		)
	}
			...
			Section {
				ColorPicker(
					...
					HStack {
						...
						if _selectedTintTheme == AppTintTheme.custom.rawValue {
							Image(systemName: "checkmark")
								.foregroundStyle(Color(hex: _customColorHex))
						}
					}
				}
				.simultaneousGesture(TapGesture().onEnded {
					_activateCustomTint(_customColorHex)
				})
			}
	...
	private func _reconcileTint() {
		let theme = AppTintTheme(rawValue: _selectedTintTheme) ?? .defaultTheme
		let hex = theme.resolvedHex(customHex: _customColorHex)
		_effectiveColorHex = hex
		UIApplication.topViewController()?.view.window?.tintColor = UIColor(Color(hex: hex))
	}

	private func _activateCustomTint(_ hex: String) {
		_selectedTintTheme = AppTintTheme.custom.rawValue
		_effectiveColorHex = hex
		UIApplication.topViewController()?.view.window?.tintColor = UIColor(Color(hex: hex))
	}
}
