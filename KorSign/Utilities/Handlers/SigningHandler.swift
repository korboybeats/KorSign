...
extension SigningHandler {
	...
	private func _locateMachosAndChangeToSDK26(for app: URL) async throws {
		if let url = Bundle(url: app)?.executableURL {
			if let error = LCPatchMachOForSDK26(url.path) {
				throw NSError(domain: "MachOPatching", code: 1, userInfo: [NSLocalizedDescriptionKey: error])
			}
		}
	}
	...
	private func _locateMachosAndFixupArm64eSlice(for app: URL) async throws {
		...
		for fileURL in machoFiles {
			...
			case "dylib":
				if let error = LCPatchMachOFixupARM64eSlice(fileURL.path) {
					throw NSError(domain: "MachOPatching", code: 1, userInfo: [NSLocalizedDescriptionKey: error])
				}
			case "framework":
				if
				...
				{
					if let error = LCPatchMachOFixupARM64eSlice(execURL.path) {
						throw NSError(domain: "MachOPatching", code: 1, userInfo: [NSLocalizedDescriptionKey: error])
					}
				}
			default:
...
enum SigningFileHandlerError: Error, LocalizedError {
	...
	var errorDescription: String? {
		...
		case .infoPlistNotFound: "Info.plist is missing, unreadable, or invalid."
