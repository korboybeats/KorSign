...
struct BackupManifest: Codable {
	...
	/// Validate the complete list before restore performs any filesystem or store writes.
		...
		for certificate in certificates {
			guard let uuid = UUID(uuidString: certificate.uuid),
				...
				throw CocoaError(.fileReadCorruptFile, userInfo: [
					NSLocalizedDescriptionKey: String.localized("The backup contains an invalid or duplicate certificate ID.")
				])
			}
		}
	}
...
final class BackupManager {
	...
	private static let _settingPrefixes = ["Feather.", "feather.", "KorSign.", "signing_options"]
	private static let _settingDenylist: Set<String> = [
		"feather.selectedCert",              // legacy indices cannot be restored safely
		"Feather.selfUpdateCertIndex",
		CertificateSelection.selectedKey,   // restored by the certificate manifest
		"KorSign.defaultImportFolderName",
		"KorSign.defaultImportFolderBookmark", // security-scoped, dead on another install
	...
	func makeBackup(password: String, components: BackupComponents) async throws -> TemporaryExport {
		...
		if components.contains(.certificates) {
			...
			selectedUUID = UserDefaults.standard.string(forKey: CertificateSelection.selectedKey)
		...
		let export = try TemporaryExport(fileName: "KorSign-Backup-\(Self._stamp()).ryukbackup")
		try packed.write(to: export.url)
		return export
	}
	...
	func restore(_ archive: BackupArchive, components: BackupComponents) throws -> BackupRestoreSummary {
		...
		if wanted.contains(.certificates) {
			...
			if
				...
				Storage.shared.getCertificate(uuid: uuid) != nil
			{
				UserDefaults.standard.set(uuid, forKey: CertificateSelection.selectedKey)
			}
		}
		...
		if wanted.contains(.tweaks), _fm.fileExists(atPath: tweaksDir.path) {
			let added = try TweakManager.shared.mergeFromBackup(tweaksDir: tweaksDir)
