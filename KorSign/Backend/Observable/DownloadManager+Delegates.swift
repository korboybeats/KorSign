...
extension DownloadManager: URLSessionDownloadDelegate {
	...
	func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
		...
		do {
			let destinationURL = try download.stageFile(at: location, suggestedFilename: downloadTask.response?.suggestedFilename)
			download.task = nil
			...
			if isAppInBackground {
				...
				showUIErrorMessage(for: download, error: error as NSError)
			}
	...
	func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
		...
		if nsError.code == NSURLErrorCancelled {
			guard !download.isPausing else { return }
