...
final class SelfUpdateManager: NSObject, ObservableObject {
	...
	func resolvedCertificate() -> CertificatePair? {
		...
		let override = defaults.string(forKey: CertificateSelection.updaterKey) ?? ""
		let uuid = override.isEmpty ? defaults.string(forKey: CertificateSelection.selectedKey) ?? "" : override
		return Storage.shared.getCertificate(uuid: uuid)
	}
	...
	private static func parse(_ json: [String: Any]) -> SelfUpdateRelease? {
		...
		let assetName = Bundle.main.bundleIdentifier == "com.korboy.korsign.dev" ? "KorSign-Dev.ipa" : "KorSign.ipa"
		let ipa = assets.first { ($0["name"] as? String) == assetName }
	...
	func beginUpdate(to release: SelfUpdateRelease) {
		_flow?.cancel()
		_flowGeneration += 1
		let generation = _flowGeneration
		_flow = Task { await performUpdate(to: release, generation: generation) }
	}
	...
	@MainActor
	private func performUpdate(to release: SelfUpdateRelease, generation: Int) async {
		guard generation == _flowGeneration, !Task.isCancelled else { return }
		...
		do {
			...
			case .server:
				...
				try checkFlow(generation)
				phase = .unverified
			case .idevice:
				try await installViaIDevice(release: release, certificate: certificate, generation: generation)
				try checkFlow(generation)
				phase = .done
			}
		} catch is CancellationError {
			guard generation == _flowGeneration else { return }
			phase = .idle
	...
	@MainActor
	private func checkFlow(_ generation: Int) throws {
	...
	@MainActor
	private func installViaServer(release: SelfUpdateRelease, certificate: CertificatePair, generation: Int) async throws {
		phase = .signing
		let installURL = try await requestServerSign(version: release.version, certificate: certificate)
		try checkFlow(generation)
		phase = .installing
		// Observe before opening: URL-open completion does not report Install vs Cancel.
		var sawPrompt = UIApplication.shared.applicationState == .inactive
		let observation = NotificationCenter.default.publisher(for: UIApplication.willResignActiveNotification)
			.sink { _ in MainActor.assumeIsolated { sawPrompt = true } }
		defer { observation.cancel() }
		guard await UIApplication.shared.open(installURL) else {
			throw NSError(domain: "SelfUpdate", code: -7, userInfo: [NSLocalizedDescriptionKey:
				String.localized("Could not open the installation request. Please try again.")])
		}
		try checkFlow(generation)
		for _ in 0..<240 {
			try checkFlow(generation)
			let state = UIApplication.shared.applicationState
			if state == .background { break }
			if sawPrompt && state == .active {
				UIApplication.shared.suspend(exitAfterSuspending: false)
				break
			}
			try await Task.sleep(nanoseconds: 250_000_000)
		}
		try await pollInstallProgress(generation: generation)
	}

	/// Progress can show activity, but disappearance and deadlines cannot prove installation.
	@MainActor
	private func pollInstallProgress(generation: Int) async throws {
	...
	@MainActor
	private func installViaIDevice(release: SelfUpdateRelease, certificate: CertificatePair, generation: Int) async throws {
		...
		phase = .downloading(0)
		let ipa = try await download(from: url, generation: generation)
	...
	private func requestServerSign(version: String, certificate: CertificatePair) async throws -> URL {
		...
		let boundary = "KorSign-\(UUID().uuidString)"
		var body = Data()
	...
	@MainActor
	private func download(from url: URL, generation: Int) async throws -> URL {
		let progress = SelfUpdateDownloadProgress { [weak self] value in
			Task { @MainActor in
				guard let self, generation == self._flowGeneration,
					case .downloading = self.phase else { return }
				self.phase = .downloading(value)
			}
		}
		let (temporary, response) = try await URLSession.shared.download(from: url, delegate: progress)
		...
		let destination = FileManager.default.temporaryDirectory
			.appendingPathComponent("SelfUpdate-\(UUID().uuidString).ipa")
		try FileManager.default.moveItem(at: temporary, to: destination)
		return destination
	}
}

/// One delegate per download; no continuation or progress handler shared between attempts.
private final class SelfUpdateDownloadProgress: NSObject, URLSessionDownloadDelegate {
