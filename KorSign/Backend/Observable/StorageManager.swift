...
enum StorageCategory: String, CaseIterable, Identifiable {
	...
	// Only these categories are disposable without knowing every operation's lifetime.
	static let safeToClear: [StorageCategory] = [.caches, .logs]
...
final class StorageManager: ObservableObject {
	...
	func delete(_ items: [StorageEntry]) {
		// Recheck at deletion time; a displayed row can predate new work or a store save.
		let library = _librarySnapshot()
		let deletable = items.filter {
			!$0.isProtected && ($0.appUuid != nil || !library.protects($0.url))
		}
	...
	func clear(_ categories: [StorageCategory]) async {
		await Task.detached(priority: .utility) {
			for category in categories { StorageScanner.clear(category) }
		}.value
	...
	private func _librarySnapshot() -> LibrarySnapshot {
		...
		// Unregistered managed files can still be in flight, and inbox files await users.
		// Explicit app deletion goes through Storage; generic file deletion cannot bypass it.
		let fm = FileManager.default
		var protected = Set([
			fm.temporaryDirectory, fm.signed, fm.unsigned, fm.certificates, fm.webManagerInbox
...
extension StorageManager {
	...
	nonisolated static func purgeStaleTemporary() {
		...
		for item in items {
			// Startup only, before operations begin. Pending downloads have no expiry:
			// age alone cannot establish that an input is no longer needed.
			guard item.lastPathComponent != staging else { continue }
...
struct LibrarySnapshot {
	...
	func protects(_ url: URL) -> Bool {
		...
		return protected.contains { stored in
			let roots = [stored, URL(fileURLWithPath: stored).resolvingSymlinksInPath().standardizedFileURL.path]
...
private enum StorageScanner {
	...
	static func scan(_ library: LibrarySnapshot) -> StorageReport {
		...
		let leftoverCount = signed.orphans.count + imported.orphans.count + certificates.orphans.count
	...
	static func clear(_ category: StorageCategory) {
		switch category {
		case .caches:
	...
	static func purge(contentsOf directory: URL) {
		for url in contents(of: directory) { try? fm.removeItem(at: url) }
	}
	...
	private static func otherPaths() -> [URL] {
		let claimed = Set(
			[fm.signed, fm.unsigned, fm.certificates, fm.archives, fm.tweaksLibrary, fm.logs]
