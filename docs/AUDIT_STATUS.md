...
## Current upstream integration (2026-09-14)

KorSign now preserves all 83 upstream commits through 844466c as its base,
with one KorSign commit retaining the original author and committer timestamps.
Both September 10 upstream commits are included in ancestry. Compatible UIKit
import pickers and deferred presentations are integrated, along with cache-file
purging; the earlier metadata, upload-list and iPad improvements remain.

Conflict resolutions retain KorSign's confirmed-only, retirement-gated install
cleanup, custom Library import/next-picker flow, and temporary-file-retaining
backup/tweak exports. The pinned Zsign reader remains instead of adopting the
additional upstream MachOReader. Existing entitlements parsing is unchanged.
Persistent global install-cleanup staging is not adopted because cleanup remains
scoped to the owning queue or batch. These are intentional fork differences.

The Mac Release build and focused picker lifecycle, batch cleanup, storage cleanup,
temporary exports and Web Manager checks passed. Main was packaged first and Dev
derived from that exact Main. Both passed archive, identities, bundled resources,
source manifest and arm64 host/widget validation. This new build has not been
phone-tested; the earlier Dev launch confirmation applies only to its prior hash.

Main SHA-256: c48bcb03be717aed2d5e692c218d4408b3f8f13efeb7128476e8142b97740943
Main size: 22,327,587 bytes. Dev stays private. Version remains 3.0.1.

## Historical selective 3.0.1 refresh (2026-09-14)

This refresh adds raw executable/icon metadata, recent-upload name deduplication,
and roomier Add Source sheets on iPad from RyukSign commit 45c01bd. Existing
installation, export and pinned Zsign protections remain intact. The broader
upstream picker and cleanup changes are not included.

The Mac Release build, metadata localization regression and Web Manager settings
checks passed. Main was packaged first; Dev was derived from that exact Main.
Both archives passed ZIP, bundle/widget identity, resources, arm64 and source
manifest checks. The user confirmed Dev opens on the phone on 2026-09-14.
This is a launch check only; the three changed behaviors were not phone-tested.

Main SHA-256: bfcbea14fdc1f75460c9650c233d84a54859267a73394e7899d2684f9b3e7426
Main size: 22,325,636 bytes. Dev remains private. Version stays 3.0.1; the embedded
build identifier remains historical, so artifact hashes identify this refresh.
The original upstream ancestry and KorSign commit timestamps are preserved.

## Historical 3.0.1 refresh (2026-09-09)
...
import on the resulting Dev IPA. These fixes are published in the source repository and Main release. This does
not mean every device edge case or optional improvement is complete.
...
or open. Some device failure paths remain unverified; see the device checklist. The
...
- Delete After Installing controls the signed library copy after confirmed
  ...
  to retired batches. The user confirmed full and partial batch cleanup;
  failure and unverified-result cases are not exhaustively device-tested.
...
| O05 | Toolchain, test and packaging maintenance | Local changes implemented and checked; Mac Release build passed. Obsolete XCTest retired. Optional hosted validation was cancelled and is not a Mac-workflow requirement; the revised hosted publication workflow remains untested. The Main release was published from the Mac. |
...
The original 18 audit fixes and the slow-import correction are included in the
published Main release. No rebuild, re-publication or repeated passed phone test
is needed to complete those changes.

Keep builds on the Mac. Select additional work only for a specific user need:
remaining device failure cases, native prompt cancellation, or the optional review
areas above. Preserve the ordinary installer and do not repeat rejected experiments.
The device checklist records the last reported phone state; publication alone does
not establish which build is currently installed.

## Outstanding validation

- Physical iPhone: deterministic failures, unverified outcomes, deleted-certificate
  recovery, updater timeout/cancellation and IDevice paths remain partly or wholly
  untested. See [device validation](DEVICE_VALIDATION.md) for exact confirmed scope.
- Mac/client integration: real Vapor response lifetimes and Finder/Windows/WebDAV
  replacement sequences are not established by the isolated filesystem fixtures.
- The retired XCTest target is not current coverage. Hosted CI was cancelled and
  is optional for the Mac-only build workflow.
- No measured performance, battery or thermal improvement is claimed. Credential
  privacy and accessibility reviews remain separate follow-up work.

Do not repeat passed tests or infer phone installation from release publication.

## Historical implementation and device records

The entries below preserve evidence from earlier checkpoints. Their branch names,
working-tree state, artifact status and proposed next steps apply only to those
checkpoints. They do not override the current summary and decisions above.
