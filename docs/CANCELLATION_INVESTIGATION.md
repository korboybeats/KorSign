...
The published 3.0.1 Main release includes the reliability fixes without adding a new
native Cancel workaround. Do not repeat the rejected hide/reopen or cancellation
inference experiments. See [device validation](DEVICE_VALIDATION.md) for confirmed and unresolved
checks and [audit status](AUDIT_STATUS.md) for exact artifact identity.

## Historical investigation records

The entries below document past builds, phone logs and rejected experiments.
"Current" and "latest" within those entries refer to that checkpoint, not the
published release. They are evidence, not instructions to rebuild or repeat tests.
The unresolved status above remains authoritative.
