...
## Build evidence for the published release: import deadline fix (2026-09-09)
...
All 18 original findings are addressed in source. Recorded confirmations apply
to the identified test builds; they do not cover every path in the release.
Artifact hashes and handoff status are recorded in AUDIT_STATUS.md.
...
At the last recorded Main updater test, the phone installed the older public
target. The public release has since been refreshed; the currently installed Main
build has not been rechecked. Dev remains separate. Updater timeout, unverified
results, cancellation and IDevice behavior have not been physically verified.

## Confirmed naming checks — reference only

These checks passed. Do not reinstall or repeat them unless a new regression
requires it. The original steps are retained for reference.
...
3. Settings → Web Manager. If the old username was `ryuk`, confirm it is now
   `korsign`. Other saved usernames stay unchanged; the literal value `ryuk` is migrated
   even if it was manually entered.

## Device procedures — use only for a selected unresolved case

These are reference procedures, not a new test assignment. The confirmed-results
section above takes precedence. Keep existing settings in mind, prepare a disposable
input, and change only the settings needed for the selected case. No new IPA handoff
...
1. Export the same signed test app twice. In Archives, confirm both copies remain.
...
KorSign reopened normally. No repeat is needed. At that time Main contained the
older target and Dev contained the tested candidate. The public Main release has since been replaced;
that publication does not prove the phone has installed it.
