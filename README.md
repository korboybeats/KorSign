...
- Optionally close the install panel automatically when finished.
- Optionally open the file picker afterward to import another IPA.
- Simplify exported IPA filenames to **App Name Version.ipa**.
- Move the Activity Logs **Share button** beside the three-dot menu.
- Fix custom theme colors not matching the selected color.
- Improve executable and icon detection (from RyukSign).
- Remove duplicate names from recent uploads (from RyukSign).
- Give Add Source more room on iPad (from RyukSign).
- Improve file-picker and sheet presentation reliability (from RyukSign).
- Clear cached files counted by Storage (from RyukSign).
...
See the [release notes](https://github.com/korboybeats/KorSign/releases/tag/v3.0.1)
for this release, [audit status](docs/AUDIT_STATUS.md) for technical evidence,
and [device validation](docs/DEVICE_VALIDATION.md) for confirmed phone checks.
