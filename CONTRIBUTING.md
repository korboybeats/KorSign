...
- The release workflow selects Xcode 26.2, checks the SDK, and validates only Main
  before publication. Local archive checks passed; the attempted hosted run was cancelled.
...
Inspect unfamiliar scripts before running them. The obsolete XCTest target and
scheme were removed; use the focused fixtures.
...
Hosted execution of this revised workflow has not yet been validated. GitHub
reported Update Repository as manually disabled on 2026-09-09; its YAML wiring
does not guarantee a feed update. The existing Main release was published from
the Mac, with its feed size updated separately.
